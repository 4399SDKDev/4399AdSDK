...
## 关于4399广告SDK
* 广告SDK使用统一一方式，从多个广告源获取广告数据，展示广告；
* 配套开发者后台（后续会开放），帮助开发者接入，查看统计数据与收入情况；
* 支持Android 平台4.0+系统；

广告SDK以本地`aar`和`library`的方式发布和接入，完整的SDK组成部分： 
```shell
4399AdSDK-v1.x.x/
├── 4399广告SDK接入文档.pdf    //本地pdf文档
...
- 开发者应用上展示广告的容器；
- 在开发者后台先注册，设定广告格式，并生成唯一的标识，即广告位的id，或`ad unit id`;
...
|开屏 |竖屏|全屏|  
|开屏 |横屏|全屏|  
|开屏 |只支持竖屏|带110dp高度底部栏的全屏|  

### 4、广告行为
- 当前支持广告的**展示、外部跳转、APK下载**行为，并将支持更多行为；
- 触发行为后，SDK将上报和统计相应事件。
...
### 准备阶段  
#### 申请应用id和广告位key
目前开发者后台不对外开放，申请广告位key和应用id，可以直接委托SDK对接者或运营。查看广告事件、收入等统计信息等后台要求，也是如此。后续将逐步开放后台。
...
dependencies {
    //建议使用最新版本
   implementation 'cn.m4399:ad.prototype:1.0.0'
}
```
...
```java
// 使用xml布局创建的横幅广告可以直接在布局文件内设置AdUnitId
// 使用方法创建的广告都有以下设置：
